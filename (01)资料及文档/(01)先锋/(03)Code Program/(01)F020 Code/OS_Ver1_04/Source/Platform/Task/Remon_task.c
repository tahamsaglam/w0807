...
***************************************/
#include "../CPU/Comm.h"
#include "./Remon_task.h"
#include "../../Controller/Key/keydecode.h"	/*新按键监视*/
#include "../../Manager/Menu/Menu.h"		/*菜单*/
#include "../../Manager/Menu/Setup.h"	/*Set菜单*/
#include "../../Common/Timer/Timer.h"	/*Timer相关*/
...
***************************************/
static const void ( * const short_2ms_task_tbl[] )( void ) =
{
	disp_function
};
...
***************************************/
static const void ( * const middle_8ms_task_tbl[] ) ( void )=
{
...
***************************************/
static const void ( * const long_16ms_task_tbl[] )(void) =
{
...
public void remon_short_func_task( void )
{
	...
	while( short_slot_cnt < (sizeof( short_2ms_task_tbl ) / sizeof ( short_2ms_task_tbl[0] )) )
	{
		short_task = short_2ms_task_tbl[short_slot_cnt];
		(*short_task)(  );/*执行函数指针指向的函数*/
		short_slot_cnt ++;
	}
	short_slot_cnt = 0;
#if 0
	for( short_slot_cnt = 0; short_slot_cnt <= sizeof( short_2ms_task_tbl )/sizeof( short_2ms_task_tbl[0] );short_slot_cnt++	 )
	{		/*运行2ms函数Tbl中所有函数*/
		short_task = short_2ms_task_tbl[short_slot_cnt];
		(*short_task)(  );/*执行函数指针指向的函数*/
		if( short_slot_cnt > sizeof( short_2ms_task_tbl ) / sizeof ( short_2ms_task_tbl[0] ) )			
		{
			short_slot_cnt = 0;
			return;
		}
...
public void remon_middle_func_task( void )
{
	...
	while( middle_slot_cnt < (sizeof( middle_8ms_task_tbl ) / sizeof ( middle_8ms_task_tbl[0] ) ))
	{
		middle_task = middle_8ms_task_tbl[middle_slot_cnt];
		(*middle_task)(  );/*执行函数指针指向的函数*/
		middle_slot_cnt ++;
	}
	middle_slot_cnt=0;
#if 0	
	for( middle_slot_cnt = 0; middle_slot_cnt <= sizeof( middle_8ms_task_tbl )/sizeof( middle_8ms_task_tbl[0] );middle_slot_cnt++	 )
	{		/*运行8ms函数Tbl中所有函数*/
		middle_task = middle_8ms_task_tbl[middle_slot_cnt];
		(*middle_task)(  );/*执行函数指针指向的函数*/
		if( middle_slot_cnt > sizeof( middle_8ms_task_tbl ) / sizeof ( middle_8ms_task_tbl[0] ) )
		{
			middle_slot_cnt = 0;
			return;
		}
...
public void remon_long_func_task( void )
{
	...
	while( long_slot_cnt < (sizeof( long_16ms_task_tbl ) / sizeof ( long_16ms_task_tbl[0] )) )
	{
		long_task = long_16ms_task_tbl[long_slot_cnt];
		(*long_task)(  );/*执行函数指针指向的函数*/
		long_slot_cnt++;
	}
	long_slot_cnt =0;
#if 0	
	for( long_slot_cnt = 0; long_slot_cnt <= sizeof( long_16ms_task_tbl )/sizeof( long_16ms_task_tbl[0] );long_slot_cnt++	 )
	{		/*运行16ms函数Tbl中所有函数*/
		long_task = long_16ms_task_tbl[long_slot_cnt];
		(*long_task)(  );/*执行函数指针指向的函数*/
//		if( long_slot_cnt >= sizeof( long_16ms_task_tbl ) / sizeof ( long_16ms_task_tbl[0] ) )
		if( long_slot_cnt > sizeof( long_16ms_task_tbl ) / sizeof ( long_16ms_task_tbl[0] ) )
		{
			long_slot_cnt = 0;
			break;
